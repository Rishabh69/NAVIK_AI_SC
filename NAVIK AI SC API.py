...
def UI(Path):
    ...
    import json
    data = {}
    data['Algorithm'] = {"Default_Value": "greedy","Selected_Value": '',"Values": [{"Display_Name": "GREEDY ALGORITHM","Value": "greedy"}                                                                                         
    ...
    data['Standardisation_Flag']={"Default_Value": 1,"Selected_Value": '',"Values": [0,1]}
    data['Primary_Column']={"Default_Value":"","Selected_Value": ''}
    data['Secondary_Column']={"Default_Value":"","Selected_Value": ''}
    import os
    wd = os.getcwd()
    wd=wd.replace('\\','/')
...
def Weight_Multiplication(Goku1,Standardised_Data,json_input,json_data_final):
    ...
    for i in range(1,len(json_data_final['Column_List'])):
...
def T_test_Func(Pairs_Final,json_data_final):
    ...
    for i in range(0,len(test_pair_final.columns)):
        Test_mean.append(round(test_pair_final[test_pair_final.columns[i]].mean(),2))

    Control_mean=[]
    for i in range(0,len(control_pair_final.columns)):
        Control_mean.append(round(control_pair_final[control_pair_final.columns[i]].mean(),2))
...
def Greedy_Calling(json_data_final):
    print("in greedy calling")
    json_input=runalgorithm_variables(json_data_final)
    print("runalgorithm completed")
    ...
    if All_Integer=='No':
        for i in range(0,len(Categorical_list)):
            ...
            T_test_Final=pd.DataFrame(np.column_stack([list(Final_categorical_list),list(control_pair_final.columns),Test_mean,Control_mean,Test_Total_Rows,Control_Total_Rows,list(np.around(np.array(list(ttest_array.pvalue)),3)),ttest]),columns=['Categorical Column','Columns','Test Mean','Control Mean','Test Count','Control Count','P Value','Pass/Fail'])
            ...
            Output_Module_1={}
            Output_Module_1['T_test'] = {"Display_Name":"T_test_Summary.csv",
        ...
        TC_pair_1=pd.DataFrame(test_control_pair_rd)
        Pairs_Final=pd.concat([Pairs_Final,TC_pair_1],axis=1)
        T_test_Final=pd.DataFrame(np.column_stack([list(control_pair_final.columns),Test_mean,Control_mean,Test_Total_Rows,Control_Total_Rows,list(np.around(np.array(list(ttest_array.pvalue)),3)),ttest]),columns=['Columns','Test_Mean','Control_Mean','Test_Count','Control_Count','P_Value','Pass/Fail'])
        T_test_Summary=pd.concat([T_test_Summary,T_test_Final])
        t = time.localtime()
        timestamp = time.strftime('%b-%d-%Y_%H%M', t)
        wd = os.getcwd()
